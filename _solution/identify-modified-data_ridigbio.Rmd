---
title: "How to identify data that have been modified by aggregator data quality processing, using R and iDigBio"
output:
  html_document:
    code_folding: show
    df_print: kable
...
Code here written by [Erica Krimmel](https://orcid.org/0000-0003-3192-0080). Please see **Use Case: [Identify modified data](https://biodiversity-specimen-data.github.io/specimen-data-use-case/use-case/identify-modified-data)** for context.
...
In this use case for the [iDigBio API](https://github.com/iDigBio/idigbio-search-api/wiki) we explore a situation where data from the provider were modified by iDigBio during ingestion as part of its data quality processing. Understanding how data were modified can help collections staff identify updates that need to be made on their specimen records as well as issues that the aggregator may need to remedy in their data quality processing.
...
Let's use the power of R to filter out data that _have not_ been modified so that we can focus on rows where the aggregator has made changes. As an example, we will look at rows where the genus name does not match between the provider and the aggregator:
...
When iDigBio makes modifications to data, these actions are recorded with *data quality flags*, for instance you will notice that all of the rows in the filtered data above have the flag "GENUS_REPLACED." We could have used values in the _flags_ field, like "GENUS_REPLACED," to search for records back at the beginning of this demo. You can learn more about the flags that iDigBio uses [here](https://github.com/iDigBio/idigbio-search-api/wiki/Data-Quality-Flags).
